...
class Utility(commands.Cog):
    ...
    async def avatar(self, ctx, *, user: discord.User = None):
        ...
        embed = discord.Embed(title=main.lang["utility_avatar_msg"].format(user.name), color=self.module_embed_color)
    ...
    async def roll(self, ctx, input_number: int = 100):
        random.seed()
        embed = discord.Embed(title=main.lang["utility_roll_msg"].format(random.randrange(input_number + 1)), color=self.module_embed_color)
        await ctx.send(embed=embed)
    ...
    async def uptime(self, ctx):
        ...
        embed = discord.Embed(title=main.lang["utility_uptime_msg"].format(bot_uptime.days,int(bot_uptime.seconds/3600),
    ...
    async def userinfo(self, ctx, *, user: discord.User = None):
        ...
        embed.add_field(name=main.lang["utility_uinfo_id"], value=user.id, inline=True)
        embed.add_field(name=main.lang["utility_uinfo_nickname"], value=user.nick, inline=True)
        embed.add_field(name=main.lang["utility_uinfo_activity"], value=user.activity, inline=True)
        embed.add_field(name=main.lang["utility_uinfo_created"], value=user_created_at, inline=True)
        embed.add_field(name=main.lang["utility_uinfo_joined"], value=user_joined_at, inline=True)
        embed.add_field(name=main.lang["utility_uinfo_sroles"].format(len(user_roles_dict)), value=user_roles, inline=False)
        await ctx.send(embed=embed)
    ...
    async def eight_ball(self, ctx, *, string: str):
        ...
        embed = discord.Embed(title=main.lang[f"utility_8ball_{ball_answer}"], color=self.module_embed_color)
        await ctx.send(embed=embed)
    ...
    async def choose_random_items(self, ctx, *, choices: str):
        items = choices.split(';')
        embed = discord.Embed(title=main.lang["utility_choose_msg"].format(secrets.choice(items).lstrip()), color=self.module_embed_color)
        await ctx.send(embed=embed)
