            ...
            if command.lower() in cmds_list or command.lower() in aliases_list:
                ...
                if not (command.description or command.help):
                    ...
                    if hasattr(command, 'permissions'):
                        embed.add_field(name=f'{lang["needed_permissions"]}:', value=', '.join(command.permissions), inline=False)
                    embed.set_footer(text=f'{lang["module_string"]}: {command.cog_name} | {lang["helpformatter_optional"]}')
            else:
