...
class LevelingNotifications(object):
    ...
    def __init__(self):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS leveling_notifications (guild_id INTEGER PRIMARY KEY, type INTEGER, channel INTEGER)")

    @classmethod
    async def change_type(self, guild_id: int, notification_type: NotificationType):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("INSERT OR IGNORE INTO leveling_notifications (guild_id) VALUES(?)", (guild_id,))
    ...
    async def set_channel(self, guild_id: int, channel_id: int):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("INSERT OR IGNORE INTO leveling_notifications (guild_id) VALUES(?)", (guild_id,))
            cursor.execute("UPDATE leveling_notifications SET type=?, channel=? WHERE guild_id=?", (int(NotificationType.Channel), channel_id, guild_id,))
            return True if cursor.rowcount > 0 else False
    ...
    async def get_guild(self, guild_id: int):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("SELECT type, channel FROM leveling_notifications WHERE guild_id=?", (guild_id,))
            output = cursor.fetchone()
