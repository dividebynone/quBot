...
class PrefixHandler(object):
    ...
    def __init__(self):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS servers(guild_id INTEGER PRIMARY KEY, prefix BLOB)")
    ...
    def get_prefix(self, guild_id: int, default_prefix: str):
        with sqlconnect(os.path.join(main.bot_path, 'databases', 'servers.db')) as cursor:
            cursor.execute("SELECT prefix FROM servers WHERE guild_id=?",(guild_id,))
