...
class Conquest(commands.Cog):
    ...
    @commands.command(name='ccreate', help=lang["command_ccreate_help"], description=lang["command_ccreate_description"], usage='"My Settlement Name" <public/private> 100', aliases=['cc'])
    ...
    @commands.command(name='cinfo', help=lang["command_cinfo_help"], description=lang["command_cinfo_description"], usage='<Settlement Name>', aliases=['ci'])
    ...
    @commands.command(name='cjoin', help=lang["command_cjoin_help"], description=lang["command_cjoin_description"], usage='<invite code> <entry fee>', aliases=['cj'])
    ...
    @commands.command(name='ccode', help=lang["command_ccode_help"], description=lang["command_ccode_description"], ignore_extra=True)
    async def ccode(self, ctx):
        ...
        if cdata and user.id in {cdata["founderid"],cdata["leaderid"]}:
            ...
            embed = discord.Embed(title= lang["conquest_code_fail"].format(cdata["invite_string"]), color = self.module_embed_color)    
        await ctx.author.send(embed=embed)

    @commands.cooldown(1, 600,commands.BucketType.user)
    @commands.command(name='cattack', help=lang["command_cattack_help"], description=lang["command_cattack_description"], usage='@somebody', ignore_extra=True)
            ...
            if cdata_defence:
                ...
                if cdata_offence:
                    ...
                    if result <= attack_score_calculated:
                        embed = discord.Embed(title=f'{cdata_offence["settlement_name"]} **VS** {cdata_defence["settlement_name"]}', colour=self.module_embed_color, description=lang["conquest_attack_result_victory"])
                        ...
                        embed = discord.Embed(title=f'{cdata_offence["settlement_name"]} **VS** {cdata_defence["settlement_name"]}', colour=self.module_embed_color, description=lang["conquest_attack_result_defeat"])
                    ...
                    embed.add_field(name=lang["conquest_win_percentage"], value=lang["conquest_chances"].format(attack_score_calculated/100), inline=True)
                    embed.add_field(name=lang["conquest_roll"], value=result, inline=True)
                    embed.add_field(name=lang["conquest_attack_wd"], value=f'  {cdata_offence["settlement_wins"]}       /      {cdata_offence["settlement_losses"]}', inline=True)
